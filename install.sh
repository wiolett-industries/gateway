...
TITLE_TAG='\033[44m\033[97m'      # blue bg, white text
...
show_summary() {
    ...
    echo -e "  ${GRAY}  First OIDC user gets 'viewer' role. Promote to admin:${NC}"
    echo -e "  ${GRAY}    docker compose exec postgres psql -U gateway -c \\${NC}"
    echo -e "  ${GRAY}      \"UPDATE users SET role='admin' WHERE email='you@example.com';\"${NC}"
    echo ""

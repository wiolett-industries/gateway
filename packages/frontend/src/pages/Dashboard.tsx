import { Globe, Lock, Award, Shield } from "lucide-react";
...
import { cn, formatDate, formatRelativeDate, daysUntil, formatTimeLeft } from "@/lib/utils";
...
function StatCard({
  ...
  const content = (
    <div className={cn("border border-border bg-card p-4 space-y-2", href && "cursor-pointer hover:bg-accent transition-colors")}>
...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (hasRole("admin")) {
      api.getAuditLog({ limit: 6 }).then((r) => setActivity(r.data || [])).catch(() => {});
    }
    ...
    api.listSSLCertificates({ status: "active", limit: 100 })
      .then((res) => {
        const expiring = (res.data || [])
          .filter((c) => c.notAfter && daysUntil(c.notAfter) <= 30 && daysUntil(c.notAfter) >= 0)
          .map((c) => ({ id: c.id, name: c.name, type: "ssl" as const, expiresAt: c.notAfter!, daysLeft: daysUntil(c.notAfter!) }));
        setExpiringItems((prev) => [...prev.filter((i) => i.type !== "ssl"), ...expiring]);
      })
      .catch(() => {});

    // Fetch expiring PKI certs
    api.listCertificates({ status: "active", limit: 100 })
      .then((res) => {
        const expiring = (res.data || [])
          .filter((c) => daysUntil(c.notAfter) <= 30 && daysUntil(c.notAfter) >= 0)
          .map((c) => ({ id: c.id, name: c.commonName, type: "pki" as const, expiresAt: c.notAfter, daysLeft: daysUntil(c.notAfter) }));
  ...
  useEffect(() => {
    const expiringCAs = (cas || [])
      .filter((ca) => ca.status === "active" && daysUntil(ca.notAfter) <= 30 && daysUntil(ca.notAfter) >= 0)
      .map((ca) => ({ id: ca.id, name: ca.commonName, type: "ca" as const, expiresAt: ca.notAfter, daysLeft: daysUntil(ca.notAfter) }));
    ...
    <PageTransition>
      <div className="h-full overflow-y-auto p-6 space-y-6">
        ...
        {expiringItems.length > 0 && (
          <div className="border bg-card" style={{ borderColor: "rgb(234 179 8 / 0.6)" }}>
            <div className="flex items-center gap-2 border-b border-border p-4">
              <h2 className="font-semibold" style={{ color: "rgb(234 179 8)" }}>Expiring Soon</h2>
              <Badge variant="warning" className="ml-auto" style={{ backgroundColor: "rgb(234 179 8)", color: "#111" }}>{expiringItems.length}</Badge>
            </div>
            <div className="divide-y divide-border">
              {[...expiringItems].sort((a, b) => a.daysLeft - b.daysLeft).map((item) => (
                <Link
                  key={`${item.type}-${item.id}`}
                  to={item.type === "ca" ? `/cas/${item.id}` : item.type === "pki" ? `/certificates/${item.id}` : "/ssl-certificates"}
                ...
                >
                  ...
                  <span className={cn(
                    "text-xs font-medium",
                    item.daysLeft <= 7 ? "text-amber-600 dark:text-amber-400 font-semibold" : "text-amber-600 dark:text-amber-400"
                  )}>
            ...
            <div className="divide-y divide-border">
              {healthHosts.slice(0, 6).map((host) => (
                <Link
                  ...
                  className="flex items-center gap-3 px-4 py-3 hover:bg-muted/50 transition-colors"
                >
                  <span className="text-sm font-medium truncate flex-1">{host.domainNames.join(", ")}</span>
                  <span className="text-xs text-muted-foreground">
                    {host.forwardHost ? `${host.forwardScheme}://${host.forwardHost}:${host.forwardPort}` : ""}
                  </span>
        ...
        <div className="border border-border bg-card">
          ...
          {(cas || []).length > 0 ? (
            <div className="divide-y divide-border">
              {(cas || []).filter((ca) => ca.status === "active").slice(0, 6).map((ca) => (
                <Link
                  key={ca.id}
                  to={`/cas/${ca.id}`}
                  className="flex items-center gap-3 px-4 py-3 hover:bg-muted/50 transition-colors"
                >
                  ...
                  <span className="text-xs text-muted-foreground">{ca.type === "root" ? "Root" : "Intermediate"}</span>

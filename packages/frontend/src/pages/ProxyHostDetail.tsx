...
export function ProxyHostDetail() {
  ...
  useEffect(() => {
    const loadRelated = async () => {
      ...
      try {
        const certRes = await api.listCertificates({ limit: 100, status: "active", type: "tls-server" });
        setPkiCerts(
          (certRes.data || []).map((c) => ({ id: c.id, commonName: c.commonName }))
        );
      } catch {
        // non-critical
      }
  ...
  const buildRequest = (): CreateProxyHostRequest => {
    ...
    const req: CreateProxyHostRequest = {
      ...
      rateLimitEnabled,
      rateLimitOptions: rateLimitEnabled ? { requestsPerSecond: rateLimitRPS, burst: rateLimitBurst } : undefined,
              ...
              <>
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                  <div className="space-y-2">
                    ...
                    <Select value={forwardScheme} onValueChange={(v) => setForwardScheme(v as ForwardScheme)}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input value={forwardHost} onChange={(e) => setForwardHost(e.target.value)} placeholder="192.168.1.100" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium">Forward Port</label>
                    <NumericInput value={forwardPort} onChange={(v) => setForwardPort(v)} min={1} max={65535} />
                  </div>
                </div>
                <div className="flex items-center gap-3">
            ...
            {type === "redirect" && (
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={String(redirectStatusCode)} onValueChange={(v) => setRedirectStatusCode(parseInt(v))}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
            ...
            {sslEnabled && (
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <Select value={sslCertificateId || "__none__"} onValueChange={(v) => setSslCertificateId(v === "__none__" ? "" : v)}>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">Internal PKI Certificate (alternative)</label>
                  <Select value={internalCertificateId || "__none__"} onValueChange={(v) => setInternalCertificateId(v === "__none__" ? "" : v)}>
                    <SelectTrigger><SelectValue placeholder="Select internal cert..." /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="__none__">None</SelectItem>
                      {pkiCerts.map((cert) => (
                        <SelectItem key={cert.id} value={cert.id}>
            ...
            <div className="space-y-3">
              ...
              {customHeaders.map((header, i) => (
                <div key={i} className="flex gap-2">
                  ...
                  />
                  <Button variant="outline" size="icon" onClick={() => setCustomHeaders(customHeaders.filter((_, j) => j !== i))}>
                    <Minus className="h-4 w-4" />
                  </Button>
                </div>
              ))}
              <Button variant="outline" size="sm" onClick={() => setCustomHeaders([...customHeaders, { name: "", value: "" }])}>
                <Plus className="h-4 w-4" />
                Add Header
              </Button>
            </div>
            ...
            <div className="space-y-3">
              ...
              {cacheEnabled && (
                ...
                >
                  ...
                  <NumericInput value={cacheMaxAge} onChange={(v) => setCacheMaxAge(v)} min={1} className="w-40" />
                </motion.div>
              )}
            </div>
              ...
              {rateLimitEnabled && (
                ...
                >
                  <div className="space-y-2">
                    ...
                    <NumericInput value={rateLimitRPS} onChange={(v) => setRateLimitRPS(v)} min={1} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm">Burst</label>
                    <NumericInput value={rateLimitBurst} onChange={(v) => setRateLimitBurst(v)} min={1} />
                  </div>
                </motion.div>
              )}
            </div>
            ...
            <div className="space-y-3">
              ...
              {customRewrites.map((rule, i) => (
                <div key={i} className="flex gap-2">
                  ...
                  <Button variant="outline" size="icon" onClick={() => setCustomRewrites(customRewrites.filter((_, j) => j !== i))}>
                    <Minus className="h-4 w-4" />
                  </Button>
                </div>
              ))}
              <Button
                variant="outline"
                size="sm"
                onClick={() => setCustomRewrites([...customRewrites, { source: "", destination: "", type: "permanent" }])}
              >
                <Plus className="h-4 w-4" />
                Add Rewrite
              </Button>
        ...
        <TabsContent value="access">
          <div className="border border-border bg-card p-6 space-y-4">
            <div className="space-y-2">
              ...
              <Select value={accessListId || "__none__"} onValueChange={(v) => setAccessListId(v === "__none__" ? "" : v)}>
            ...
            {healthCheckEnabled && (
              ...
              >
                <div className="space-y-2">
                  ...
                  <Input value={healthCheckUrl} onChange={(e) => setHealthCheckUrl(e.target.value)} placeholder="/" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium">Interval (seconds)</label>
                  <NumericInput value={healthCheckInterval} onChange={(v) => setHealthCheckInterval(v)} min={5} max={3600} className="w-40" />
                </div>
              </motion.div>
            )}

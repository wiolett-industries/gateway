  ...
  const handleSave = async () => {
    ...
    try {
      if (isNew) {
        ...
        const t = await api.createNginxTemplate({ name, description: description || undefined, type, content, variables: validVars });
        toast.success("Template created");
        navigate(`/nginx-templates/${t.id}`, { replace: true });
      } else {
        const validVars = variables.filter((v) => v.name.trim());
        await api.updateNginxTemplate(id!, { name, description: description || null, content, variables: validVars });
    ...
    <div className="h-full overflow-y-auto p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {isNew ? "Define a Handlebars nginx server block template" : isBuiltin ? "Built-in template (read-only)" : "Edit nginx config template"}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={handleTest} disabled={!content.trim() || isTesting}>
      ...
      {/* Test result banner */}
      {testResult && (
        <div className={`flex items-center gap-2 p-3 border ${testResult.valid ? "border-green-600/30 bg-green-600/5" : "border-destructive/30 bg-destructive/5"}`}>
          {testResult.valid ? (
            <><Check className="h-4 w-4 text-green-600" /><span className="text-sm text-green-600">nginx -t passed</span></>
          ) : (
            <><X className="h-4 w-4 text-destructive" /><span className="text-sm text-destructive">nginx -t failed: {testResult.errors.join(", ")}</span></>
          )}
          <Button variant="ghost" size="icon" className="h-6 w-6 ml-auto" onClick={() => setTestResult(null)}>
            <X className="h-3 w-3" />
          </Button>
        </div>
      )}
      ...
      <div className="flex gap-4">
        <div className="flex-1 space-y-1">
          ...
          <Input value={name} onChange={(e) => setName(e.target.value)} disabled={isBuiltin} placeholder="Template name" />
        </div>
        <div className="flex-1 space-y-1">
          <label className="text-xs font-medium text-muted-foreground">Description</label>
          <Input value={description} onChange={(e) => setDescription(e.target.value)} disabled={isBuiltin} placeholder="Optional description" />
        </div>
        <div className="w-40 space-y-1">
          <label className="text-xs font-medium text-muted-foreground">Host Type</label>
          <Select value={type} onValueChange={(v) => setType(v as ProxyHostType)} disabled={!isNew}>
      ...
      <div className="flex gap-4">
        ...
        {showReference && (
          <div className="w-72 shrink-0 border border-border bg-card overflow-y-auto" style={{ maxHeight: "540px" }}>
            <div className="p-3 border-b border-border">
              <h3 className="text-sm font-semibold">Variable Reference</h3>
            </div>
            <pre className="p-3 text-xs text-muted-foreground whitespace-pre-wrap">{VARIABLE_REFERENCE}</pre>
          </div>
        )}
      </div>
        ...
        <div className="border border-border bg-card">
          <div className="flex items-center justify-between p-3 border-b border-border">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Define variables that proxy hosts can fill in when using this template. Use them as <code className="bg-muted px-1">{"{{variableName}}"}</code> in the template.</p>
            </div>
            <Button variant="outline" size="sm" onClick={() => setVariables([...variables, { name: "", type: "string", description: "" }])}>
            ...
            <div className="p-3 space-y-2">
              {variables.map((v, i) => (
                <div key={i} className="flex items-center gap-2">
                  <Input
                    value={v.name}
                    onChange={(e) => { const next = [...variables]; next[i] = { ...next[i], name: e.target.value }; setVariables(next); }}
                    placeholder="Variable name"
                    className="w-40 font-mono text-xs"
                  />
                  <Select
                    value={v.type}
                    onValueChange={(val) => { const next = [...variables]; next[i] = { ...next[i], type: val as 'string' | 'number' | 'boolean' }; setVariables(next); }}
                  >
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      next[i] = { ...next[i], default: v.type === "number" ? (raw ? Number(raw) : undefined) : v.type === "boolean" ? raw === "true" : (raw || undefined) };
                      setVariables(next);
                    }}
                    placeholder="Default"
                  ...
                  <Input
                    value={v.description || ""}
                    onChange={(e) => { const next = [...variables]; next[i] = { ...next[i], description: e.target.value || undefined }; setVariables(next); }}
                    placeholder="Description"
                    className="flex-1 text-xs"
                  />
                  <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" onClick={() => setVariables(variables.filter((_, j) => j !== i))}>
      ...
      {showPreview && (
        <div className="border border-border bg-card">
          ...
          {isPreviewLoading ? (
            ...
            <pre className="p-4 text-xs font-mono overflow-x-auto whitespace-pre">{previewResult}</pre>
          )}
        </div>
      )}
    </div>

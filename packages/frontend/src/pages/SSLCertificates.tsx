...
import { formatDate, daysUntil, hoursUntil, cn } from "@/lib/utils";
    ...
    <div className="h-full overflow-y-auto p-6 space-y-2">
      ...
      <SearchFilterBar
        ...
        filters={
          <>
            <div className="w-40">
              <Select value={filters.type} onValueChange={(v) => setFilters({ type: v as SSLCertType | "all" })}>
            ...
            <div className="w-40">
              <Select value={filters.status} onValueChange={(v) => setFilters({ status: v as SSLCertStatus | "all" })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                ...
                {certificates.map((cert) => {
                  ...
                  return (
                    <tr key={cert.id} className="hover:bg-accent transition-colors">
                      ...
                      <td className="p-3"><SSLTypeBadge type={cert.type} /></td>
                      <td className="p-3"><SSLStatusBadge status={cert.status} /></td>
                      <td className="p-3">
                        {cert.notAfter ? (
                          <span className={cn(
                            "text-sm",
                            expDays !== null && expDays <= 7 ? "text-red-600 dark:text-red-400 font-medium" :
                            expDays !== null && expDays <= 30 ? "text-yellow-600 dark:text-yellow-400" :
                            ...
                            {expDays !== null && expDays === 0 && cert.notAfter && hoursUntil(cert.notAfter) > 0 && (
                              <span className="text-xs ml-1">({hoursUntil(cert.notAfter)}h)</span>
                            )}
                          </span>
                        ) : (
